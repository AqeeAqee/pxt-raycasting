...
function createSprite(x: number, y: number, vx: number, vy: number, textures: Image[][], kind: number) {
    ...
    // rcRender.startEffect(spr, effects.fire, 1000)
    // spr.startEffect(effects.fountain, 100000)
    // if(spr.id>8)
    spr.sayText( spr.id+"哈喽, baby!", 88888, true)
    return spr
}
...
let duck=createSprite(8, 7, 0, 0, texturesDuck, SpriteKind.Enemy)
createSprite(6, 7, 0, 11, texturesDonut, SpriteKind.Enemy)
let cake = createSprite(3, 3, -9, -6, texturesBigCake, SpriteKind.Enemy) //[[img`0`]]
cake.setStayInScreen(true)
// cake.setFlag(SpriteFlag.RelativeToCamera, true)
// cake.startEffect(effects.fountain, 100000)
...
duck.startEffect(effects.fire, 1000000)

rcRender.takeoverParticleSources()
